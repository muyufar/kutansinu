...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // Validasi komprehensif pemesanan bus
...
<div class="container mt-4">
    ...
    <div class="row">
        <div class="col-md-4">
            ...
            <div class="card mt-3" id="jadwal-card">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">Jadwal Bus yang Tersedia</h5>
                </div>
                <div class="card-body" id="jadwal-content">
                    <?php
                    // Ambil jadwal bus yang tersedia untuk tanggal yang dipilih
                    $selected_date = isset($_POST['tanggal_berangkat']) ? $_POST['tanggal_berangkat'] : date('Y-m-d');
                    $available_schedule = getAvailableBusSchedule($db, $selected_date);

                    if (!empty($available_schedule)): ?>
                        <div class="alert alert-success">
                            <strong>✅ Tersedia untuk Dipesan:</strong> Berikut adalah bus yang tersedia untuk tanggal <?= date('d/m/Y', strtotime($selected_date)) ?>
                        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Update jadwal bus yang tersedia saat tanggal berubah
        tanggalInput.addEventListener('change', function() {
            updateAvailableSchedule(this.value);
        });
    });
    const BUS_ID = <?= json_encode($bus_id) ?>;

    // Fungsi untuk update jadwal bus yang tersedia
 function updateAvailableSchedule(tanggal) {
        if (!tanggal) return;

        const scheduleCard = document.getElementById('jadwal-content');
        if (scheduleCard) {
            scheduleCard.innerHTML = '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Memuat...</div>';
        }

        fetch(`get_available_schedule.php?tanggal=${encodeURIComponent(tanggal)}&bus_id=${BUS_ID}`)
            .then(response => response.text())
            .then(html => {
                if (scheduleCard) scheduleCard.innerHTML = html;
            })
            .catch(err => {
                console.error('Error:', err);
                if (scheduleCard) {
                    scheduleCard.innerHTML = '<div class="alert alert-danger">Gagal memuat jadwal.</div>';
                }
            });
    }
    ...
    // Panggil saat halaman dimuat jika sudah ada tanggal
    document.addEventListener('DOMContentLoaded', function() {
        const tanggalInput = document.getElementById('tanggal_berangkat');
        if (tanggalInput.value) {
            updateAvailableSchedule(tanggalInput.value);
        }
    });
</script>